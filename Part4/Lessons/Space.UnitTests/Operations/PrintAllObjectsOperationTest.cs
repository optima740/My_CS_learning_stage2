...
namespace Space.UnitTests.Operations
{
    ...
    public class PrintAllObjectsOperationTest
    {
        ...
        [Test]
        public async Task AllPrintOperation_ShouldPrintAllObjects()
        {
            //Arrange                          
            _repositoryMock.Setup(repo => repo.GetAllByTypeAsync<SpaceObject>()).Returns(_objectsCollection.ToAsyncEnumerable());

            //Act
            await _printAllOperation.DoOperationAsync();
        ...
        [Test]
        public async Task PrintModelNotContains_ShouldntPrintModelAttributes()
        {
            //Arrange            
            var spaceObjectsCollection = new List<SpaceObject> { };
            _repositoryMock.Setup(repo => repo.GetAllByTypeAsync<SpaceObject>()).Returns(spaceObjectsCollection.ToAsyncEnumerable());
